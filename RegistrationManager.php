...
class RegistrationManager
{
	...
	function checkLoginDetails($username,$password)
	{
		...
		$query = "
			SELECT *, p.profileID AS pid, d.* 
			FROM 
				table_profile p
			LEFT JOIN 
				table_driverID d ON p.profileID=d.profileID
	...
	function newVehicle($owner,$uname,$vehicletype,$plateno,$model,$year,$motor,$chassis,$color,$laststickeryear,$laststicker,$reference,$certReg,$receiptReg,$ltfrbFranchise,$insurance,$deed)
	{
		...
		$query = "
			INSERT INTO
				table_vehicle (plateNumber, vehicleType, owner, model, year, motor, chassis, color, reference, certificationRegistration, receiptRegistration, LTFRBFranchise, insurance, deed)
